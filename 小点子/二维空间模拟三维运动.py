...
for i in range(20000):  # while True:  # 无限循环用这个
    ...
    while True:
        if (
            np.abs(x1 - axis_x) > observation_max
            or np.abs(x2 - axis_x) > observation_max
            or np.abs(x3 - axis_x) > observation_max
            or np.abs(y1 - axis_y) > observation_max
            or np.abs(y2 - axis_y) > observation_max
            or np.abs(y3 - axis_y) > observation_max
        ):
            observation_max = observation_max * 2  # 有一个物体超出视线时，坐标翻倍
        elif (
            np.abs(x1 - axis_x) < observation_max / 10
            and np.abs(x2 - axis_x) < observation_max / 10
            and np.abs(x3 - axis_x) < observation_max / 10
            and np.abs(y1 - axis_y) < observation_max / 10
            and np.abs(y2 - axis_y) < observation_max / 10
            and np.abs(y3 - axis_y) < observation_max / 10
        ):
    ...
    if np.mod(i, 1000) == 0:
        if not os.path.exists("three-body/"):
            os.makedirs("three-body/")
